...
FROM alpine:${APLINE_VERSION} AS glibc_download
ARG GLIBC_VERSION
ARG BASE_URL=https://ftp.gnu.org/gnu
ARG GLIBC=glibc_${GLIBC_VERSION}.tar.gz
ARG GLIBC_URL=${BASE_URL}/glibc/${GLIBC_FILENAME}
ARG GNU_KEYRING=gnu_keyring.gpg
...
RUN apk add --no-cache curl gnupg && \
    ...
    gpg --verify --keyring ./${GNU_KEYRING} && \
    tar -xzf ${GLIBC}
...
RUN mkdir -p ${ABUILD_KEY_DIR} && \
    ...
    sudo openssl rsa -in ${ABUILD_KEY_DIR}\key.pem -pubout -out /etc/apk/keys/glibc.rsa.pub && \
    echo "PACKAGE_PRIVKEY=\"${ABUILD_KEY_DIR}/glibc-key.pem\"" > ${ABUILD_KEY_DIR}/abuild.conf && \
    sed -i "s/<pkgver>/${GLIBC_VERSION}/
            s/<glibc-sha512sum>/$(cat glibc-bin-${GLIBC_VERSION}.sha512sum | awk '{print $1}')/" APKBUILD && \
    abuild -r
